...
def return_answer_candidate(df, query): # query를 바로 embedding해서 넣는 작업
  ...
  top_three_doc = df.sort_values("similarity", ascending=False).head(1) # 코사인 유사도가 가장 높은 상위 3개의 데이터 찾아 반환
...
with st.form('form', clear_on_submit=True):
   user_input = st.text_input('한국통계정보원의 규정에 대해 물어보세요!', '', key='input')
